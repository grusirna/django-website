...
from website.tools.dutils import RelatedObject
...
def xstatic(*tags):
    ...
    '''
    from website.views.configs import vendors
    node = vendors
        ...
        try:
            ...
            if tag.startswith('website'):
                ...
                if file_type in ('css', 'js'):
                    node = "website/%s/%s" % (file_type, tag)
                else:
                    raise e
            else:
                raise e
...
def quote(s):
    """
    Ensure that primary key values do not confuse the website URLs by escaping
...
def flatten_fieldsets(fieldsets):
    """Returns a list of field names from an website fieldsets structure."""
    field_names = []
...
def get_deleted_objects(objs, opts, user, website, using):
    """
    ...
    def format_callback(obj):
        has_admin = obj.__class__ in website.modelconfigs
        opts = obj._meta
        ...
        if has_admin:
            ...
            p = '%s.delete_%s' % (opts.app_label,
                                  opts.object_name.lower())
            if not user.has_perm(p):
                perms_needed.add(opts.verbose_name)
            # Display a link to the website page.
            return mark_safe('<span class="label label-info">%s:</span> <a href="%s">%s</a>' %
                             (escape(capfirst(opts.verbose_name)),
                              admin_url,
                              escape(obj)))
        else:
            # Don't display link to edit, because it either has no
            # website or is edited inline.
            return mark_safe('<span class="label label-info">%s:</span> %s' %
                             (escape(capfirst(opts.verbose_name)),
                              escape(obj)))
...
def get_fieldname(verbose_name, model):
    fields = [a for a in model._meta.fields if a.verbose_name == verbose_name]
    print(fields)
    if fields:
...
def display_for_field(value, field):
    from website.views.configs import EMPTY_CHANGELIST_VALUE
...
def get_model_from_relation(field):
    ...
    elif not getattr(field, 'remote_field'):
        return field.model
