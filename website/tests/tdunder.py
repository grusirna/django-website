...
from website.tools.dunder import Collection, Q
from website.tools.dunder import dunderkey, dunder_partition, dunder_init, dunder_last, \
    dunder_get, undunder_keys, dunder_truncate, QuerySet, filter_items, lookup, include_keys, LookupyError
    ...
    assert_list_equal(ik(entries, ['request']),
                      [{'request': {'url': 'http://example.com',
                       ...
                       {'request': {'url': 'http://example.org',
                                    'headers': [{'name': 'Connection', 'value': 'Keep-Alive'}]}},
                       {'request': {'url': 'http://example.com/myphoto.jpg',
                                    'headers': [{'name': 'Connection', 'value': 'Keep-Alive'}]}}])
...
def test_undunder_keys():
    entry = {'request__url': 'http://example.com',
...
class TestDunder(TestCase):
    def testddk(self):
        self.assertEqual(dunderkey('a', '1', 'b'), 'a__1__b')
        self.assertEqual(dunderkey('a', '1', 'b', 'c', 'd__e__f'), 'a__1__b__c__d__e__f')
        self.assertEqual(dunder_partition('a__1__b__c'), ('a__1__b', 'c'))
        self.assertEqual(dunder_init('a__1__b__c'), 'a__1__b')
        self.assertEqual(dunder_last('a__1__b__c'), 'c')
        self.assertEqual(dunder_get({'a': 1}, 'a'), 1)
        self.assertEqual(dunder_get({'a': {'b_c': {'c': 1}}}, 'a__b_c__c'), 1)
        self.assertEqual(undunder_keys({'a__b': 1}), {'a': {'b': 1}})
        self.assertEqual(dunder_truncate({'a__b': 1}), {'b': 1})
        self.assertEqual(dunder_truncate({'a__b': [1], 'c__d': 2}), {'b': [1], 'd': 2})
        self.assertEqual(dunder_truncate({'a__b': 1, 'c__b': 2}), {'a__b': 1, 'c__b': 2})
        self.assertEqual(dunder_truncate({'a__p': 'yay', 'b__p': 'no', 'c__z': 'dunno'}),
    ...
    def testdd(self):
        entry = {'request__url': 'http://example.com',
                 ...
                 'response__status': 404,
                 'response__headers': [{'name': 'Date', 'value': 'Thu, 13 Jun 2013 06:43:14 GMT'}]}
        self.assertEqual(dunder_truncate(entry),
                         {'url': 'http://example.com',
                          'request__headers': [{'name': 'Connection', 'value': 'Keep-Alive', }],
                          'status': 404,
                          'response__headers': [{'name': 'Date', 'value': 'Thu, 13 Jun 2013 06:43:14 GMT'}]})
...
class TestQuery(TestCase):
    def testc(self):
        a = QuerySet([{'a': 1, 'b': {'c': 2}}]).filter(b__c=2)
        self.assertEqual(len(a), 1)
        a = QuerySet([{'a': 1, 'b': {'c': 2}}]).filter(Q(b__c=2))
        self.assertEqual(len(a), 1)
        self.assertEqual(QuerySet([{'a': 1, 'b': {'c': 2}}]).filter(Q(b__c=2) | Q(a=1))[0], {'a': 1, 'b': {'c': 2}})
        self.assertEqual(QuerySet([{'a': 1, 'b': {'c': 2}}]).filter(Q(b__c=2) | Q(a=2))[0], {'a': 1, 'b': {'c': 2}})
        self.assertNotEqual(QuerySet([{'a': 1, 'b': {'c': 2}}]).filter(Q(b__c=2) & Q(a=2)), 1)
        self.assertEqual(a[0], {'a': 1, 'b': {'c': 2}})
        a = QuerySet([{'a': 1, 'b': {'c': 2}}]).select('b__c')
        self.assertEqual(a[0], {'b': {'c': 2}})
        a = QuerySet([{'a': 1, 'b': {'c': 2}}]).filter(b__c=2)
        self.assertEqual(QuerySet([{'a': 'small', 'b': {'c': 2}}]).filter(a__startswith='sma')[0],
                         {'a': 'small', 'b': {'c': 2}})

    def testk(self):
        self.assertEqual(list(include_keys([{'a': 1}, {'a': 2}], 'a')), [{'a': 1}, {'a': 2}])
        self.assertEqual(list(include_keys([{'a': 1}, {'a': {'c': 1}}], 'a')), [{'a': 1}, {'a': {'c': 1}}])
        self.assertNotEqual(list(include_keys([{'a': 1}, {'a': {'c': 1}}], 'a__c')), [])
