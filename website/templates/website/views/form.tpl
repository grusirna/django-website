{% extends base_template %}
{% load i18n %}

{% load website_tags %}
{% load crispy_forms_tags %}

{% block nav_title %}{% if menu_icon %}<i class="{{menu_icon}}"><sub class="fa fa-{% if add%}plus{%else%}pencil{%endif%}"></sub></i> {% endif %}
{{ title }}{% endblock %}

{% block nav_btns %}
{{ nav_buttons }}
{% endblock nav_btns %}

{% block content %}
<form class="exform" {% if has_file_field %}enctype="multipart/form-data" {% endif %}action="{{ form_url }}" method="post" id="{{ opts.model_name }}_form">{% csrf_token %}
  {% block form_top %}{% endblock %}
  {% plugin_block 'form_top' %}

  {% if errors %}
      <div class="alert alert-danger alert-dismissable">
        <button type="button" class="close" data-dismiss="alert">&times;</button>
      {% blocktrans count counter=errors|length %}Please correct the error below.{% plural %}Please correct the errors below.{% endblocktrans %}
      </div>
      {{ form.non_field_errors }}
      <div style="display: none">
          {% for error in errors %}
            {{ error.name }}{{ error.errors }}
          {% endfor%}
      </div>
  {% endif %}

  {% plugin_block 'before_fieldsets' %}

  {% crispy form %}

  {% plugin_block 'after_fieldsets' %}

  <div class="form-actions well well-sm clearfix">
  <button type="submit" class="default btn btn-primary" name="_save" data-loading-text="{% trans "Saving.." %}"/><i class="fa fa-save"></i> {% trans 'Save' %}</button>
  <input type="submit" class="btn btn-default" value="{% trans 'Save and continue editing' %}" name="_continue" />
  {% plugin_block 'submit_line' %}
  </div>

</form>
{% endblock %}
